...
export const getAuthorizeUrl = async (req: Request, res: Response) => {
    try {
        ...
        const config = OAUTH_CONFIGS[providerLower as keyof typeof OAUTH_CONFIGS];
        ...
        if ((config as any).isMarketplace) {
...
export const handleCallback = async (req: Request, res: Response) => {
    try {
        ...
        if (!tokenResponse.ok) {
            console.error(`OAuth token exchange failed for ${provider}:`, await tokenResponse.text());
 ...
 */
export const saveApiKeyConnection = async (req: Request, res: Response) => {
    try {
 ...
 */
export const getProviderAuthType = async (req: Request, res: Response) => {
    try {
        const { provider } = req.query;
