...
export class AnalyticsService {
     ...
     */
    static async snapshotUserSavings(userId: string, connectionId?: string) {
        try {
            ...
            if (connectionId) {
                const connectionResults = await ScanResult.find({ userId, connectionId });
                await this.createSnapshot(userId, today, connectionResults, connectionId);
            }

            // 2. Snapshot Aggregate for the user (all connections)
            const allResults = await ScanResult.find({ userId });
            await this.createSnapshot(userId, today, allResults);
    ...
    private static async createSnapshot(userId: string, date: Date, results: any[], connectionId: string | null = null) {
        const zombies = results.filter(r =>
        ...
        await SavingsHistory.findOneAndUpdate(
            { userId, date, connectionId: connectionId || null },
            {
                userId,
                date,
     ...
     */
    static async getUserSavingsHistory(userId: string, days: number = 30, connectionId: string | null = null) {
        ...
        const history = await SavingsHistory.find(query).sort({ date: 1 });
    ...
    static async updateCommunityStats() {
        try {
            ...
            const allSavings = await SavingsHistory.aggregate([
                { $match: { date: today } },
                {
                    $group: {
                        _id: null,
            ...
            const topServices = await ScanResult.aggregate([
    ...
    static async getLeaderboard() {
        ...
        const monthlyStats = await CommunityStats.aggregate([
        ...
        // Get all-time stats
        const allTimeStats = await CommunityStats.aggregate([
            {
                $group: {
                    _id: null,
                    totalSavings: { $sum: '$totalSavings' }
